...
echo -e "  ${CYAN}Proxy addon${NC}  $INSTALL_DIR/lw-proxy/addon.py"
echo -e "  ${CYAN}mitmproxy  ${NC}  $(command -v mitmproxy)"
echo -e "  ${CYAN}CA cert    ${NC}  $CA_DEST"
echo ""
echo -e "${YELLOW}  Quick start:${NC}"
echo -e "${DIM}    # Set your target${NC}"
echo -e "    TARGET=\"https://target.example.com\""
echo ""
echo -e "${DIM}    # Create sessions for each privilege level${NC}"
echo -e "    lw session new --name admin --role admin"
echo -e "    lw session new --name guest --role user"
echo ""
echo -e "${DIM}    # Terminal 1 — capture admin traffic through the proxy pointed at your target${NC}"
echo -e "    mitmdump -s $INSTALL_DIR/lw-proxy/addon.py \\"
echo -e "        --set ww_session=admin --set ww_db=webweaver_staging.db -p 8080"
echo -e "    ${DIM}# Configure your browser proxy to 127.0.0.1:8080 then browse \$TARGET as admin${NC}"
echo ""
echo -e "${DIM}    # Terminal 2 — sync + run all attack modules${NC}"
echo -e "    lw sync --staging webweaver_staging.db --watch &"
echo -e "    lw run-all --session-a admin --session-b guest --output findings.json"
echo ""
echo -e "${DIM}    # Dry run IDOR scan (no requests sent)${NC}"
echo -e "    lw scan-idor --session-a admin --session-b guest --dry-run"
echo ""
echo -e "${DIM}    # Generate a HackerOne report${NC}"
echo -e "    lw generate-report --session-a admin --session-b guest \\"
echo -e "        --format hackerone --output report.md"
echo ""
